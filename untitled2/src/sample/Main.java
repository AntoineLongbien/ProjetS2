...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        //################## FIN SALLE 3 #####################
        // VIDEO DE FIN
        StackPane finsc = new StackPane();
        MediaPlayer player2 = new MediaPlayer(new Media(getClass().getResource("endvideo.mp4").toExternalForm()));
        MediaView mediaView2 = new MediaView(player2);
        Image endim=new Image(getClass().getResource("end.png").toExternalForm());
        ImageView endimg=new ImageView(endim);
        finsc.getChildren().add(mediaView);
        Scene scenefin = new Scene(finsc, 1280, 720);
        //################################
        ...
        salle3.setOnMouseClicked((evt) -> {
            ...
            if (evt.getX() < 899 && evt.getX() > 766 && evt.getY() < 556 && evt.getY() > 430 && noticeup == false&& finale==false) {
                for (int i = 0; i < 4; i++) {
                    if (salle3.getChildren().contains(becs[i])) {
                        salle3.getChildren().remove(becs[i]);
                    }
            ...
            if(finale==true && (evt.getX() < 994 && evt.getX() > 883 && evt.getY() < 599 && evt.getY() > 430)){
                ...
                finsc.getChildren().add(titmus);
                finsc.getChildren().add(endimg);
                finsc.getChildren().add(mediaView2);
